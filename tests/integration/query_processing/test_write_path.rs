...
#[turso_macros::test(views)]
fn test_dependent_materialized_views_reload_in_dependency_order(
    tmp_db: TempDatabase,
) -> anyhow::Result<()> {
    let db_path = tmp_db.path.clone();
    let conn = tmp_db.connect_limbo();

    conn.execute("CREATE TABLE raw_events (id INTEGER PRIMARY KEY, event_type TEXT)")?;
    conn.execute("CREATE MATERIALIZED VIEW z_upstream AS SELECT * FROM raw_events")?;
    conn.execute(
        "CREATE MATERIALIZED VIEW a_downstream AS SELECT id FROM z_upstream WHERE event_type = 'file_edited'",
    )?;
    conn.execute("INSERT INTO raw_events VALUES (1, 'file_edited')")?;
    conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")?;
    ...
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT * FROM a_downstream ORDER BY id"),
        vec![vec![RValue::Integer(1)]]
    );

    conn.execute("INSERT INTO raw_events VALUES (2, 'file_edited')")?;
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT * FROM a_downstream ORDER BY id"),
